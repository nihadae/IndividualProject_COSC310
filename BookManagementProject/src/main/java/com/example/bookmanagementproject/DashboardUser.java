...
public class DashboardUser implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        String quoteMain = QuoteGet.getData().get(0);
        String author = QuoteGet.getData().get(1);
        if (author.equals("zenquotes.io")){
            author = "anonymous";
        }
        quote.setText("\""+quoteMain+"\" by " + author);
    }
    ...
    public void quizPressed(){
        try{
            Parent root = FXMLLoader.load(getClass().getResource("quizViewUser.fxml"));
            Stage stage = new Stage();
            Scene scene = new Scene(root);
            stage.setScene(scene);
            stage.show();
